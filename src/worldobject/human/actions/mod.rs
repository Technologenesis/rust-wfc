...
use crate::{
    world::{World, handle::WorldObjectHandle}, worldobject::{
        WorldObject, components::controllable::controller::commands::Command, fns::update::Action, human::Human
    }
};
...
impl Human {
    pub fn from_command(&mut self, cmd: Command, world: &World, my_handle: WorldObjectHandle) -> Result<Action, CommandToActionError> {
        match cmd {
            Command::Move(move_cmd) => Ok(move_action::from_command(move_cmd, my_handle, todo!())),
            ...
            Command::Attack(attack_cmd) => attack_action::from_command(attack_cmd, world, todo!())
            ...
            Command::Inventory => Ok(inventory_action::action(todo!())),
